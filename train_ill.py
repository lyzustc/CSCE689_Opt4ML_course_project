...
from resnet import new_resnet50
from torch.utils.tensorboard import SummaryWriter

import sogclr.new_builder
...
parser.add_argument('--loss_coeff', default='0.01', type=float,
                    help='coefficient for the intermediate-layer loss')
parser.add_argument('--jig_prob', default='0.0', type=float,
                    help='probability for jigsaw augmentation')
parser.add_argument('--gauss_prob', default='0.0', type=float,
                    help='probability for gaussian blur augmentation')
parser.add_argument('--solar_prob', default='0.0', type=float,
                    help='probability for solarization augmentation')
parser.add_argument('--jig_num', default='2', type=int,
                    help='the image will be split into num*num patches')
parser.add_argument('--use_new_aug', default=False, type=bool,
                    help='whether use new augmentation strategy or not')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = sogclr.new_builder.NewSimCLR_ResNet(
            partial(new_resnet50, zero_init_residual=True), 
    ...
    if args.use_new_aug:
        augmentation1 = [
            transforms.RandomApply([
                sogclr.loader.Jigsaw(n=args.jig_num)
            ], p=args.jig_prob),
            ...
            transforms.RandomApply([sogclr.loader.GaussianBlur([.1, 2.])], p=args.gauss_prob),
            transforms.RandomApply([sogclr.loader.Solarize()], p=args.solar_prob),
...
def train(train_loader, model, optimizer, scaler, summary_writer, epoch, args):
    ...
    save_root_path = args.save_dir
    total_loss, total_last_loss = 0.0, 0.0
    for i, (images, _, index) in enumerate(train_loader):
        ...
        with torch.cuda.amp.autocast(True):
            loss_list = model(images[0], images[1], index, args.gamma)
            loss = loss_list[-1] + args.loss_coeff * sum(loss_list[:-1]) / (len(loss_list) - 1)
            last_loss = loss_list[-1]

        losses.update(loss.item(), images[0].size(0))
        total_loss += loss.item()
        total_last_loss += last_loss.item()
    ...
    file = open(os.path.join(save_root_path, 'record.txt'), 'a')
    file.write("Epoch {} | Average mean loss over all layers {} loss only last layer {}\n".format(epoch, total_loss / (i + 1), total_last_loss / (i + 1)))
    file.close()
