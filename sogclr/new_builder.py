...
class NewSimCLR(nn.Module):
    """
    Build a SimCLR model with a base encoder, and two MLPs
   
    """
    def __init__(self, base_encoder, dim=256, mlp_dim=2048, T=1.0, cifar_head=False, loss_type='dcl', N=50000, num_hiddens=4, num_proj_layers=2, device=None):
        """
        ...
        if self.loss_type == 'dcl':
            self.u = [torch.zeros(N).reshape(-1, 1) for _ in range(num_hiddens)] #.to(self.device) 
    ...
    def dynamic_contrastive_loss(self, hidden1, hidden2, u_index, index=None, gamma=0.99, distributed=True):
        ...
        u1 = (1 - gamma) * self.u[u_index][index].cuda() + gamma * torch.sum(neg_logits1, dim=1, keepdim=True)/(2*(batch_size-1))
        u2 = (1 - gamma) * self.u[u_index][index].cuda() + gamma * torch.sum(neg_logits2, dim=1, keepdim=True)/(2*(batch_size-1))
        
        self.u[u_index][index] = u1.detach().cpu()+ u2.detach().cpu()
    ...
    def forward(self, x1, x2, index, gamma):
        ...
        if self.loss_type == 'dcl':
            for i in range(len(h1)):
                loss_list.append(self.dynamic_contrastive_loss(h1[i], h2[i], i, index, gamma))
        elif self.loss_type == 'cl':
            for i in range(len(h1)):   
                loss_list.append(self.contrastive_loss(h1[i], h2[i]))
        return loss_list


class NewSimCLR_ResNet(NewSimCLR):
